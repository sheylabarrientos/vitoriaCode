...
class Elevador(
    ...
    fun adicionarPessoa(): Int { //A função foi setada do tipo "Int".
        return if (numeroPessoas < numeroMaxPessoas) { //"Se o "numeroPessoas" (que foi setado inicialmente como "0") for menor que o "numeroMaxPessoas" (setado inicialmente como "5"), então..."
            numeroPessoas++ //"Soma-se "1" ao "numeroPessoas"..."
            numeroPessoas //"E retorna o resultado de "numeroPessoas"".
        } else {
    ...
    fun excluirPessoa(): Int {
        return if (numeroPessoas >= 1) { //"Se o "numeroPessoas" (que foi setado inicialmente como "0") for maior ou igual a "1", então..."
